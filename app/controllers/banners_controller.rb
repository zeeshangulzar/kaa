class BannersController < ApplicationController
  ...
  def create
    ...
    if (@current_user.location_coordinator? && @current_user.location_ids.include?(banner.location_id)) || (@current_user.sub_promotion_coordinator_or_above? && @current_user.promotion_id == @promotion.id) || @current_user.master?
  ...
  def destroy
    banner = banner.find(params[:id]) rescue nil
    return HESResponder("Banner", "NOT_FOUND") if !banner
    if (@current_user.sub_promotion_coordinator_or_above? && @current_user.promotion_id == banner.promotion_id) || @current_user.master?
      Banner.transaction do
        banner.destroy
      end
    else

    ...
    if oauth_token
      ...
      User.transaction do
        ...
        if user.profile.started_on >= user.promotion.current_date
          notification.update_attributes :title => "Fitbit Connected", :message=>"Your Fitbit data will sync with <i>Go KP</i> starting #{user.profile.started_on.strftime('%B %e')}."
        else
          notification.update_attributes :title => "Fitbit Connected", :message=>"Your Fitbit data will sync with <i>Go KP</i> shortly."
        end

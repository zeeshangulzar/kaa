class GoMailer < ActionMailer::Base
  ...
  def challenge_received_email(challenge_sent, receiver)
    ...
    @user = receiver
    mail(:to => @user.email, :subject => "#{Constant::AppName}: New challenge from #{@challenge_sent.user.profile.full_name}", :from => fromHandler(@challenge_sent.user))
  end
  ...
  def daily_email(day, promotion, to_name, to_email, base_url, user)
    ...
    mail(:to => to, :subject => "#{Constant::AppName}: #{@tip.title}", :from => from, :reply_to => reply_to)

namespace :resque do
  task :restart do
    ...
    cmd = "(cd #{Dir.pwd} && nohup nice -5 bundle exec rake environment resque:work RAILS_ENV=production HES_SECURITY_DISABLED=true QUEUE=image_processing,default,* PIDFILE=log/resque.pid >> 'log/resque.log' 2>&1 &) && sleep 1"
    puts cmd
    system(cmd)
    puts "Should be up."
  end
